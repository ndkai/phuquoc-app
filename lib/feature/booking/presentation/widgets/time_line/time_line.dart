...
class MyTimeLine extends StatefulWidget {
  ...
  final ValueChanged<DateByServiceAndUnit>? onChangeDay;
  const MyTimeLine({Key? key,this.activeDate, this.onChangeDay, this.currentMonth, this.curentYear, this.availableDay}) : super(key: key);

  @override
  _MyTimeLineState createState() => _MyTimeLineState();
}
GlobalKey<_MyTimeLineState> timeLineKey = new GlobalKey<_MyTimeLineState>();
...
class _MyTimeLineState extends State<MyTimeLine> {
  ...
  void changeDate(int month, int year){
    setState(() {
      ...
      days.clear();
      DateTime lastDayOfMonth = new DateTime(curentYear!, currentMonth! + 1, 0);
      print("dđd ${lastDayOfMonth}");
      for(var i = 1; i <= lastDayOfMonth.day; i++){
        print("dđ ${i}");
        days.add(DateTime(curentYear!,currentMonth!, i));
      }
      ...
      var container = DateTimeCardItem(
        ...
        onTap: (e){
          setState(() {
            ...
            if(dateByServiceAndUnit != null){
              widget.onChangeDay!(dateByServiceAndUnit);
            }
